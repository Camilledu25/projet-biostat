...
p<-ggplot(coeur, aes(x=hauteConsomation, y=graisse, color=hauteConsomation)) +
  geom_boxplot()
p
```
...
```{r}
coeur$MCC<-as.factor(coeur$MCC)
p<-ggplot(coeur, aes(x=MCC, y=consommation, color=MCC)) +
  geom_boxplot()
p
```
```{r}
table(coeur$emploi,coeur$MCC)
barplot(prop.table(table(coeur$emploi,coeur$MCC),margin=2),
beside=TRUE, ylim=c (0 ,1),legend.text = T,
main="Proportion des filières dans chaque lycée ",xlab ="Filière",ylab="Fréquence")
```
